...
chipperText = ''

#print 'len chip ',len(chipperText)

# 13 6 5 7 3 4 9 10 12 1 2 8 11
kunci = [13 ,6 ,5 ,7, 3, 4, 9, 10, 12, 1, 2, 8, 11] 
...
def calculate_fitness(bigramProb, trigramProb):
    ...
    for i in range(len(pop)):
        ...
        for (bi, tri) in zip(bestBigram[i], bestTrigram[i]):
            ...
            if (tri not in trigram_list) or (tri not in bestTrigram):
